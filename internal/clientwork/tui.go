...
func (app *TUI) getImage() image.Image {
	b, err := base64.StdEncoding.DecodeString(beach)
	if err != nil {
...
func (app *TUI) authPage(message string) {
	...
	form.AddImage("\"GoKeeper\"", app.getImage(), 0, 14, 16777216)
	...
	form.AddButton("Login", func() {
		...
		if errors.Is(err, storage.ErrUnknown) || err != nil {
			...
			app.authPage("Something is wrong. ;(")
			return
		}

		app.recordsInfoPage("Login successfully!")
	})
	...
	form.AddButton("Register", func() {
		...
		if errors.Is(err, storage.ErrLoginExists) {
			...
			app.authPage("Login exists. Please try again.")
			return
		}
		if errors.Is(err, handlers.ErrEmptyField) {
			log.Infoln(handlers.ErrEmptyField)
...
func (app *TUI) recordsInfoPage(message string) {
	...
	listFrame := tview.NewFrame(list).SetBorders(0, 0, 0, 1, 4, 4).
		AddText(
			"Up/Down - switch records / Enter - choose option",
			false,
			tview.AlignLeft,
			tcell.ColorWhite,
		).
...
func (app *TUI) setNewAESKey(message string) {
	...
	form.AddButton("Set AES key", func() {
		...
		if errors.Is(err, handlers.ErrEmptyField) {
			...
			app.authPage("AES field is empty.")
			return
		}
		if errors.Is(err, storage.ErrUnknown) || err != nil {
			log.Infoln(storage.ErrUnknown)
		...
		app.recordsInfoPage("AES key change successfully!")
	})
...
func (app *TUI) recordPage(recordID string, message string) {
	...
	if err != nil {
		...
		app.recordsInfoPage("Failed get record. Wrong AES key???")
		return
	}
	...
	frame.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		...
		case tcell.KeyCtrlD:
			...
			if errors.Is(err, storage.ErrUnknown) || err != nil {
				...
				app.recordPage(recordID, "Something is wrong. ;(")
				return event
			}
...
func (app *TUI) createTextRecord() {
	...
	form.AddButton("OK", func() {
		...
		if errors.Is(err, storage.ErrUnknown) {
			...
			app.recordsInfoPage("Something is wrong. ;(")
			return
		}
		if errors.Is(err, handlers.ErrWrongAESKey) {
			app.authPage("Wrong master key. Please login again.")
...
func (app *TUI) createCardRecord() {
	...
	form.AddTextView("", "Please, use real number and exp date credit card (regex used!)", 0, 0, false, false)
	...
	form.AddButton("OK", func() {
		// Regex for credit card - exp date
		...
		// Regex for credit card - card number
		result, err = regexp.Match(
		...
		// Regex for credit card - card CVC
...
func (app *TUI) createFileRecord() {
	...
	form.AddInputField("Filepath", "", 60, nil, func(text string) {
		file.FilePath = text
	})
